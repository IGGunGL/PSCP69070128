...
```text
Input : บรรทัดที่ 1: ตัวอักษรหนึ่งตัว M ระบุสถานะสมาชิก ('Y' หรือ 'N') ตัวใหญ่หมด
บรรทัดที่ 2 : จํานวนเต็ม n โดยที่ n > 0
...
```text
Step 1: รับสถานะสมาชิก is_member ('Y' หรือ 'N') และจำนวนรายการสินค้า n
Step 2: วนลูปรับราคาสินค้าทีละรายการจำนวน n ครั้ง แล้วนำมาบวกสะสมเป็นยอดรวม total
Step 3: ตรวจสอบเงื่อนไขส่วนลด
```
...
```text
ทำเหมือนแผนแรก แต่ใช้ Decimal แทน float เพื่อให้คำนวณเงินและปัดเศษทศนิยมได้แม่นยำ ไม่เพี้ยนตามที่เพื่อนเตือนเรื่องการปัดเศษ
```
...
```text
เช็กว่าถ้าเป็นสมาชิก (Y) ระบบจะลด 5% ทันที แม้ยอดซื้อจะไม่ถึง 500 บาท
```
...
```text
เช็กเงื่อนไขไม่ใช่สมาชิก (N) แต่ยอดรวมถึงเกณฑ์ 500 บาทพอดี ว่าได้รับส่วนลด 3% ถูกต้องไหม
```
...
```text
เช็กเงื่อนไขที่ไม่เข้าพวก (else) ว่าต้องจ่ายราคาเต็ม ไม่มีการหักส่วนลด
```
