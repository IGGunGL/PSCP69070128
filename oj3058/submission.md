...
```text
input = a b goal โดย a อิฐ 1 นิ้ว b อิฐ 5 นิ้ว goal จำนวนที่่ต้องใ้ช
output = ค่า a 
constraints = โดยก่อนจะปริ้น a ออกมา ต้องใช้ b ให้มากสุดก่อนค่อยใช้ a โดยถ้าสร้างไม่ได้ให้ตอบ -1
```
...
```text
Step 1: อ่านจำนวนอิฐเล็ก a, อิฐใหญ่ b และความยาวเป้าหมาย goal
Step 2: หาจำนวนอิฐใหญ่ที่จะใช้ (big_used) โดยใช้เท่าที่มี (b) หรือใช้เท่าที่จำเป็น (goal // 5)
Step 3: คำนวณจำนวนอิฐเล็กที่ต้องใช้เพิ่ม (small_need = goal - (big_used * 5))
Step 4: ตรวจว่ามีอิฐเล็กพอหรือไม่ (a >= small_need)
Step 5: ถ้าพอให้พิมพ์ small_need ถ้าไม่พอให้พิมพ์ -1
```
...
```text
เหมือนกับแผนแรกเพราะแต่เช็คบาง testcase นิดหน่อย
```
...
```text
เช็กการใช้อิฐใหญ่ร่วมกับอิฐเล็กได้พอดี
```
...
```text
เช็กกรณีอิฐไม่พอสร้างสะพานจนจบ
```
...
```text
เช็กกรณีใช้อิฐเล็กล้วน เมื่อไม่มีอิฐใหญ่เลย
```
