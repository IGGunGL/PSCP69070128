...
```text
input = 1:  หมายเลขห้องเริ่มต้น
Output:  จำนวนกำแพงน้อยที่สุดที่ต้องพังเพื่อไปยังห้อง 1
```
...
```text
Step 1: อ่านจำนวนเต็ม n_room (หมายเลขห้อง)
Step 2: คำนวณชั้น r โดยใช้สูตร math.isqrt(n_room - 1) + 1
Step 3: คำนวณลำดับตำแหน่งห้อง k ในชั้นนั้น โดยนำ n_room - (r - 1)**2
Step 4: ตรวจว่า k เป็นเลขคี่หรือไม่ (k % 2 == 1)
Step 5: ถ้าจริง ให้คำนวณจำนวนกำแพง walls = 2 * (r - 1)
Step 6: ถ้าไม่จริง ให้คำนวณจำนวนกำแพง walls = 2 * (r - 1) - 1
Step 7: พิมพ์ค่า walls ออกทางหน้าจอ
```
...
```text
วิธีสุดท้ายเหมือนแผนแรก ใช้ Modulo (% 2) เช็กเลขคู่-คี่ และได้ทดสอบ 0 และเลขติดลบ ตามคำแนะนำของเพื่อน
```
...
```text
ทดสอบว่าโปรแกรมจัดการกรณีห้องแรกสุด
```
...
```text
ทดสอบประสิทธิภาพและขอบเขตสูงสุดที่โจทย์กำหนด
```
