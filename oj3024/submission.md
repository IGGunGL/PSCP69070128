...
```text
Input: คะแนนรวม 3 คน  และคะแนนสูงสุด

Output: Surprising หรือ Not surprising

Condition: หาคะแนนต่ำสุด แล้วเช็กว่า bestvote - total > 2
```
...
```text
Step 1: รับค่าจากผู้ใช้
        allvote = input()
        bestvote = input()
Step 2: คำนวณคะแนนต่ำสุดที่ยอมรับได้
        min_vote = bestvote - 2
Step 3: หาคะแนนรวมขั้นต่ำที่ไม่น่าประหลาดใจ
        min_allvote = bestvote + min_vote
Step 4: ตรวจสอบคะแนนรวม
        ถ้า allvote <= min_allvote
        -> พิมพ์ "Not surprising"
        ถ้าไม่ใช่
        -> พิมพ์ "Surprising"
```
...
```text
Step 1: อ่านค่า allvote และ bestvote
Step 2: หาคะแนนต่ำสุดที่เป็นไปได้ของคนที่สาม
        total = allvote - (2 * bestvote)
Step 3: ตรวจสอบเงื่อนไขคะแนนติดลบ
        ถ้า total < 0 ให้ปรับ total = 0
Step 4: ตรวจสอบความห่างของคะแนน
        ถ้า (bestvote - total) > 2
        -> พิมพ์ "Surprising"
        ถ้าไม่ใช่ 
        -> พิมพ์ "Not surprising"
```
...
```text
คอยช่วยสอนพวก edge case
```

คุณยังทำอะไรด้วยตนเอง

```text
เขียนแผน เขียนโค้ด ทดสอบและส่ง oj ด้วยตนเอง
```
