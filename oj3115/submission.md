...
```text
input : บรรทัดที่ 1 : รับ num (จำนวนร้าน) และ check (จำนวนเวลาที่ตรวจ)
บรรทัดที่ 2 ถึง num+1: รับ start และ stop (เวลาเปิด-ปิดของแต่ละร้าน)
บรรทัดที่ 3 : รับเวลาที่ต้องการตรวจ check ค่า (คั่นด้วยช่องว่าง)
output : ผลลัพธ์ check ค่า แต่ละค่าคือจำนวนร้านที่เปิดอยู่ในเวลานั้นๆ โดยพิมพ์ต่อกันในบรรทัดเดียว คั่นด้วยช่องว่าง
```
...
```text
Step 1: รับจำนวนร้านค้า num และจำนวนคำถาม check
Step 2: รับเวลาเปิด-ปิดของแต่ละร้าน แล้วมาร์กจุดเปิด (+1) และจุดปิด (-1) ในอาร์เรย์บอกการเปลี่ยนแปลง (diff)
Step 3: วนลูปบวกสะสม (Prefix Sum) เพื่อคำนวณจำนวนร้านที่เปิดในแต่ละนาที (0-1440) ไว้ล่วงหน้า
Step 4: รับเวลาที่ต้องการตรวจทั้งหมด แล้วดึงตัวเลขร้านที่เปิดตามนาทีนั้นออกมาพิมพ์คั่นด้วยช่องว่าง
```
...
```text
ทำตามแผนแรกเลย โดยเลือกใช้เทคนิค Difference Array คู่กับ Prefix Sum เพราะช่วยคำนวณจำนวนร้านที่เปิดไว้รอได้ในรอบเดียว ดึงคำตอบได้ทันที ไม่ต้องวนลูปซ้อนให้ช้าตามที่เพื่อนแนะนำ
```
...
```text
เช็กว่าเวลามีร้านเปิดทับซ้อนกัน ระบบบวกสะสมจำนวนร้านได้ถูกต้อง
```
...
```text
เช็กว่า ณ นาทีที่ปิดร้าน (stop) ร้านนั้นต้องไม่ถูกนับแล้ว
```
...
```text
เช็กกรณีเวลาที่ตรวจอยู่นอกช่วงเวลาของทุกร้าน ต้องตอบ 0
```
