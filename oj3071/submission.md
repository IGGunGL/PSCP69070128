...
```text
input : เลข 2 ตัว ตัวหาร และเศษ
output : จำนวนตัวที่ตรงกับเศษ
```
...
```text
Step 1: อ่านค่าจำนวนเต็ม num1, num2, d, และ r
Step 2: ตั้งค่าตัวนับ count = 0
Step 3: วนลูปให้ตัวแปร i มีค่าตั้งแต่ num1 ถึง num2 (รวม num2)
Step 4: ในแต่ละรอบ ตรวจสอบว่า i % d == r หรือไม่
Step 5: ถ้าจริง ให้บวกค่า count เพิ่มขึ้น 1 (count += 1)
Step 6: เมื่อวนลูปครบทุกตัว ให้พิมพ์ค่า count ออกมา
```
...
```text
ใช้เหมือนวิธีแรกเพราะโจทย์ไม่ค่อยซับซ้อน
```
...
```text
ทดสอบการทำงานพื้นฐานกับช่วงจำนวนเต็มบวกทั่วไป
```
