...
```text
Input: เดือน และ วันที่ 

Output: ชื่อฤดู หรือ error

Constraints: ต้องเป็น จำนวนเต็ม (int) เท่านั้น
```
...
```text
Step 1: อ่านจำนวนเต็ม month และ day
Step 2: ตรวจว่าตรงกับเงื่อนไขฤดูหนาว (winter) หรือไม่ 
        (ม.ค., ก.พ. หรือช่วงปลาย ธ.ค. / ต้น มี.ค.)
        -> ถ้าจริง พิมพ์ "winter"
Step 3: ถ้าไม่ใช่ ตรวจว่าตรงกับเงื่อนไขฤดูใบไม้ผลิ (spring) หรือไม่
        (เม.ย., พ.ค. หรือช่วงปลาย มี.ค. / ต้น มิ.ย.)
        -> ถ้าจริง พิมพ์ "spring"
Step 4: ถ้าไม่ใช่ ตรวจว่าตรงกับเงื่อนไขฤดูร้อน (summer) หรือไม่
        (ก.ค., ส.ค. หรือช่วงปลาย มิ.ย. / ต้น ก.ย.)
        -> ถ้าจริง พิมพ์ "summer"
Step 5: ถ้าไม่ใช่ ตรวจว่าตรงกับเงื่อนไขฤดูใบไม้ร่วง (fall) หรือไม่
        (ต.ค., พ.ย. หรือช่วงปลาย ก.ย. / ต้น ธ.ค.)
        -> ถ้าจริง พิมพ์ "fall"
Step 6: ถ้าไม่ตรงกับข้อไหนเลย ให้พิมพ์ "error"
```
...
```text
เหมือนกันแผนแรกเนื่องจากโจทย์ไม่ค่อยมีความซับซ้อน
```
...
```text
เช็กวันที่ 21 ว่าเปลี่ยนฤดูได้รึเปล่า
```
...
```text
เช็กว่าถ้าใส่ข้อมูลผิด จะ error รึเปล่า
```
