...
```text
input : บรรทัดที่ 1 รับค่า : ต้นทาง ปลายทาง (ภาษาอังกฤษตัวพิมพ์ใหญ่เท่านั้น)
บรรทัดที่ 2 รับค่า : น้ำหนักพัสดุ
output : แสดงผลราคาการนําส่งพัสดุที่ได้จากข้อมูลนําเข้า
constraints : หากเส้นทางที่ระบุไม่อยู่ในตารางให้แสดงข้อความว่า Error)
```
...
```text
Step 1: รับค่าชื่อเมืองต้นทาง ปลายทาง และน้ำหนักพัสดุ
Step 2: เช็กว่าต้นทางกับปลายทางตรงกับคู่เมืองที่กำหนดไว้หรือไม่
Step 3: ถ้าตรง ให้คิดราคาตามสูตรของเส้นทางนั้น แล้วแสดงผลเป็นทศนิยม 2 ตำแหน่ง
Step 4: ถ้าไม่ตรงกับคู่เมืองไหนเลย ให้แสดงผลว่า "Error"
```
...
```text
วิธีสุดท้ายของฉันเหมือนกับแผนแรก ฉันใช้ if-elif ตรวจสอบคู่เมืองต้นทางและปลายทางทีละคู่ แล้วคำนวณราคาตามสูตรของเส้นทางนั้นโดยแสดงผลทศนิยม 2 ตำแหน่งด้วย f-string หากไม่ตรงกับเส้นทางใดเลยก็ให้เข้า else แล้วพิมพ์ "Error" ฉันทดสอบกรณีที่ป้อนชื่อเมืองที่ไม่มีในตารางและตัวพิมพ์เล็ก-ใหญ่ด้วยตนเองหลังจากเพื่อนเตือนให้ระวังเคสที่อยู่นอกเหนือเงื่อนไข
```
...
```text
กรณีเส้นทางย้อนกลับที่ไม่มีในเงื่อนไข
```
